...
!        h2form = 1.0d-17*dsqrt(initialTemp)
        h2form = 3e-18*sqrt(initialTemp)*exp(-initialTemp*1e-3)
...
!        write(10,8000)
        ...
        if (.false.) then !IF (readAbunds .eq. 0) THEN
        ...
        !choose species you're interested in by looking at parameters.f90
            write(10,8030) timeinyears,dens(dstep),temp(dstep),av(dstep),abund(outIndx,dstep)
            8030  format(1pe10.3,1x,1pe10.3,1x,1pe10.3,1x,1pe10.3,20(1x,1pe10.3))
...
!        h2form = 1.0d-17*dsqrt(temp(dstep))
        h2form = 3e-18*sqrt(temp(dstep))*exp(-temp(dstep)*1e-3)
...
!        ENDIF
!        h2col = abund(nh2,dstep)*coldens(dstep)
!        cocol = abund(nco,dstep)*coldens(dstep)
